...
export const loginService = async (c: Context) => {
    ...
    try {
        ...
        if (user.auth === false) return c.json({ message: "please verify your email" }, 404);
...
export const SignUp = async (c: Context) => {
    ...
    if (!parse.success) return c.json({ message: "all data and password validation are essansitial" }, 404);
    try {
...
export const verifyUser = async (c: Context) => {
    ...
    try {
        ...
        if (user.auth === true) return c.json({ message: "User already verified" }, 404);
